...
void field_types_bcf_style()
{
    ...
    std::get<std::vector<std::vector<int8_t>>>(compare_recs[1].genotypes().back().second).push_back({-128});
    std::get<std::vector<std::vector<int8_t>>>(compare_recs[2].genotypes().back().second).push_back({-128});
    std::get<std::vector<std::vector<int8_t>>>(compare_recs[3].genotypes().back().second).push_back({-128});
...
void field_types_vcf_style()
{
    ...
    // this workaround is pending clarification in https://github.com/samtools/hts-specs/issues/593
    compare_recs[1].genotypes().second.back().push_back(std::vector<int8_t>{-128});
    compare_recs[2].genotypes().second.back().push_back(std::vector<int8_t>{-128});
    compare_recs[3].genotypes().second.back().push_back(std::vector<int8_t>{-128});
