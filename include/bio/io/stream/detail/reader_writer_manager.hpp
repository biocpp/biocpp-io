...
#include <bio/io/stream/detail/latch.hpp>
...
private:
    ...
    public:
        ...
        ~scoped_writer_type() { manager.writer_arrive(); }
        //!\}
    ...
    public:
        ...
        ~scoped_reader_type() { manager.reader_arrive(); }
        //!\}
...
public:
    ...
    /*!\brief Constructs the reader_writer_manager with the reader count, writer count and the associated data
     * structure. \tparam concurrent_t The type of the concurrent data structure. Must have a member function `close`.
    ...
    template <typename concurrent_t>
        ...
        requires requires
        {
            std::declval<concurrent_t>().close();
        } // requires closable concurrent data structure.
    //!\endcond
    reader_writer_manager(size_t const rcount, size_t const wcount, concurrent_t & ds) :
      reader_latch{static_cast<ptrdiff_t>(rcount)},
      writer_latch{static_cast<ptrdiff_t>(wcount)},
     ...
     * Thread-safe.
     */
    void reader_arrive_and_wait() noexcept { reader_latch.arrive_and_wait(); }
     ...
     * Thread-safe.
     */
    void reader_arrive() noexcept { reader_latch.arrive(); }
     ...
     * Thread-safe.
     */
    scoped_writer_type register_writer() noexcept { return scoped_writer_type{*this}; }
     ...
     * Thread-safe.
     */
    scoped_reader_type register_reader() noexcept { return scoped_reader_type{*this}; }

private:
    //!\brief The internal latch for consumer threads.
    alignas(64) latch reader_latch;
    //!\brief The internal latch for producer threads.
    alignas(64) latch writer_latch;
