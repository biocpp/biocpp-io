...
#include <bio/io/stream/detail/spin_delay.hpp>
...
template <std::semiregular value_t,
          typename buffer_t                 = std::vector<value_t>,
...
public:
    ...
    private:
         ...
         * `position % buffer.size()`. The current implementation uses a trick to compute the modulo by means of a
         * bitwise and-operation which is supposedly faster than modulo. To do so, the position is masked with
         ...
         * With this implementation trick it is possible to compute the modulo to get the respective ring buffer
         * position using the function bio::io::contrib::buffer_queue::to_buffer_position.
         *
         * ### Example
         *
         * Assume the buffer has at the moment space for 12 elements. Accordingly, the ring_buffer_capacity is set
         * to 16. If the next ring buffer write position is 12 we need a wrap around in the ring-buffer in order to
         * start at position 0 of the actual underlying buffer instead. To do so the remaining difference between buffer
         * size and ring buffer capacity is added such that the postion is now a multiple of `ring_buffer_capacity`.
...
template <std::semiregular value_t, typename buffer_t = std::vector<value_t>>
