...
/*!\file
 * \brief Provides bio::io::txt::writer.
...
/*!\brief A concept that describes types directly writable via txt
 * \ingroup txt
 *
 * \details
 *
...
//!\ingroup txt
template <txt::record_kind record_kind_ = txt::record_kind::line>
...
private:
    ...
    void write_single(txt::writable_to_output auto && arg)
    {
...
public:
    ...
    plaintext_output_iterator(std::ostream & ostr, char const sep)
      //!\cond REQ
      requires(record_kind_ == txt::record_kind::line_and_fields)
      //!\endcond
      :
    ...
    plaintext_output_iterator & operator=(std::string_view const line)
      //!\cond REQ
      requires(record_kind_ == txt::record_kind::line)
    //!\endcond
    {
        write_line(line);
        return *this;
    ...
    template <typename range_of_fields_t>
        //!\cond REQ
        requires((record_kind_ == txt::record_kind::line_and_fields) && std::ranges::input_range<range_of_fields_t> &&
                 txt::writable_to_output<std::ranges::range_reference_t<range_of_fields_t>>)
    //!\endcond
    ...
    //!\brief Writes a character to the associated output stream.
    plaintext_output_iterator & operator=(txt::record const & record)
      //!\cond REQ
      requires(record_kind_ == txt::record_kind::line_and_fields)
    //!\endcond
    {
    ...
    void write(txt::writable_to_output auto && head, txt::writable_to_output auto &&... tail)
    {
        write_single(std::forward<decltype(head)>(head));
        if constexpr (sizeof...(tail) > 0)
        {
            if constexpr (record_kind_ == txt::record_kind::line)
    ...
    void write_line(txt::writable_to_output auto &&... args)
    {
 ...
 * Line-wise writing:
 * ```cpp
 * bio::io::txt::writer writer{"example.txt"};
 *
 ...
 * Column-wise writing:
 *```cpp
 * bio::io::txt::writer writer{"example.tsv.gz", '\t'}; // transparent compression supported
 *
...
public:
     ...
     * \details
     *
     * See bio::io::txt::writable_to_output for a list of the types supported.
     *
     ...
     *
     * The element type of the range must be one of the types described in bio::io::txt::writable_to_output.
     *
