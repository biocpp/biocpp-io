...
/*!\file
 * \brief Provides utility types for bio::io::txt::reader and bio::io::txt::writer.
...
/*!\brief The value type of bio::io::txt::reader if every line is split into fields.
 * \details
 *
 * Plain I/O records are always shallow.
 *
 * Note that if you are reading line-wise (not splitting on delimiters), the value type
 * of bio::io::txt::reader is just a std::string_view and not of this type.
 */
struct record
{
...
/*!\brief A helper for specifying the header of a bio::io::txt::reader.
 ...
 *
 *   * bio::io::txt::header_kind::none -> no header
 *   * bio::io::txt::header_kind::first_line -> first line is treated as header
 *   * bio::io::txt::header_kind::starts_with{c} -> all lines starting with character "c" are header
 *
 * ### Example
 *
 * ```cpp
 * bio::io::txt::reader r{"example.txt"};                                              // Implicitly no header
 * bio::io::txt::reader r{"example.txt", bio::io::txt::header_kind::none};             // Explicitly no header
 * bio::io::txt::reader r{"example.txt", bio::io::txt::header_kind::first_line};       // First line
 * bio::io::txt::reader r{"example.txt", bio::io::txt::header_kind::starts_with{'#'}}; // Lines starting with
 * '#'
 * ```
 */
class header_kind
