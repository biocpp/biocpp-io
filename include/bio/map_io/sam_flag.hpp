 ...
 *   when the tools in use are aware of this bit. It is typically used to flag alternative mappings when multiple
 * mappings are presented in a SAM.
 * * bio::map_io::sam_flag::supplementary_alignment (bit `0x800`) indicates that the corresponding alignment line is
 * part of a chimeric alignment. If the SAM/BAM file corresponds to long reads (nanopore/pacbio) this happens when reads
 * are split before being aligned and the best matching part is marked as primary, while all other aligned parts are
 ...
 *   bio::map_io::sam_flag::proper_pair, bio::map_io::sam_flag::secondary_alignment, and
 * bio::map_io::sam_flag::supplementary_alignment (bits `0x2`, `0x100`, and `0x800`).
 * * bio::map_io::sam_flag::on_reverse_strand (bit `0x10`) indicates  whether the read sequence has been reverse
 * complemented and the quality string is reversed.  When bit bio::map_io::sam_flag::unmapped (`0x4`) is unset, this
 *   corresponds to the strand to which the segment has been mapped: bio::map_io::sam_flag::on_reverse_strand (bit
 * `0x10`) unset indicates the forward strand, while set indicates the reverse strand. When
 * bio::map_io::sam_flag::unmapped (`0x4`) is set, this indicates whether the unmapped read is stored in its original
 * orientation as it came off the sequencing machine.
 * * bio::map_io::sam_flag::first_in_pair and bio::map_io::sam_flag::second_in_pair (bits `0x40` and `0x80`) reflect the
 * read ordering within each template inherent in the sequencing technology used. If
 * bio::map_io::sam_flag::first_in_pair and bio::map_io::sam_flag::second_in_pair (`0x40` and `0x80`) are both set, the
 * read is part of a linear template, but it is neither the first nor the last read. If both are unset, the index of the
 * read in the template is unknown. This may happen for a non-linear template or when this information is lost during
 * data processing.
 * * If bio::map_io::sam_flag::paired (bit `0x1`) is unset, no assumptions can be made about
 * bio::map_io::sam_flag::proper_pair, bio::map_io::sam_flag::mate_unmapped,
 * bio::map_io::sam_flag::mate_on_reverse_strand, bio::map_io::sam_flag::first_in_pair and
