...
private:
    ...
    void decode_numbers_into(std::span<std::byte const> in, out_t & out)
    {
        ...
        std::byte const * cache_end_ptr = cache_ptr + in.size();
    ...
    var_io::genotypes_bcf_style<ownership::deep>    deep_genotypes_cache;
    //!\brief This is only used as temporary storage when reading VCF style genotypes.
    var_io::genotypes_bcf_style<ownership::shallow> shallow_genotypes_cache;
    ...
    //!\brief Reading of INFO field.
    template <ownership own, typename id_t>
    void parse_info_impl(std::vector<std::pair<id_t, var_io::dynamic_type<own>>> & parsed_field)
    {
        ...
        for (size_t i = 0; i < record_core->n_info; ++i)
        {
            ...
            var_io::dynamic_type<own> variant;
            ...
            if constexpr (std::integral<id_t>)
                ...
                id = header.infos[header.idx_to_info_pos().at(idx)].id;

            parsed_field.emplace_back(std::move(id), std::move(variant));
        }
    ...
    void parse_field(vtag_t<field::info> const & /**/,
                     std::vector<std::pair<id_t, var_io::dynamic_type<ownership::shallow>>> & parsed_field)
    {
        parse_info_impl<ownership::shallow>(parsed_field);
    }

    //!\overload
    template <typename id_t>
    void parse_field(vtag_t<field::info> const & /**/,
                     std::vector<std::pair<id_t, var_io::dynamic_type<ownership::deep>>> & parsed_field)
    {
        parse_info_impl<ownership::deep>(parsed_field);
    }
    ...
    {
        using field_t = std::remove_cvref_t<decltype(parsed_field)>;
        constexpr ownership own =
          std::same_as<field_t, var_io::genotypes_bcf_style<ownership::shallow>> ? ownership::shallow : ownership::deep;
        ...
        for (size_t i = 0; i < static_cast<size_t>(record_core->n_fmt); ++i)
        {
            ...
            if (format.id == "GT") // this needs custom decoding, it is not a string
            {
                ...
                for (size_t sample = 0; sample < record_core->n_sample; ++sample)
                {
                    if constexpr (own == ownership::shallow)
    ...
    void parse_field(vtag_t<field::genotypes> const & /**/,
                     var_io::genotypes_bcf_style<ownership::shallow> & parsed_field)
    {
        parse_genotypes_impl(parsed_field);
    }

    //!\overload
    void parse_field(vtag_t<field::genotypes> const & /**/, var_io::genotypes_bcf_style<ownership::deep> & parsed_field)
    {
        parse_genotypes_impl(parsed_field);
    }
    ...
    void parse_genotypes_impl_vcf_style(auto & genotypes_cache, auto & parsed_field)
    {
        using field_t = std::remove_cvref_t<decltype(parsed_field)>;
        constexpr ownership own =
          std::same_as<field_t, var_io::genotypes_vcf_style<ownership::shallow>> ? ownership::shallow : ownership::deep;

        using id_t = std::conditional_t<own == ownership::shallow, std::string_view, std::string>;

        genotypes_cache.clear();
        parse_genotypes_impl(genotypes_cache);
        ...
        for (auto & [idx, dyn_type] : genotypes_cache)
        {
            ...
            parsed_formats.push_back(static_cast<id_t>(format.id));
        }
        ...
        for (size_t s = 0; s < record_core->n_sample; ++s)
        {
            ...
            for (size_t f = 0; f < parsed_formats.size(); ++f)
            {
                ...
                std::visit(visitor, genotypes_cache[f].second);
            }
        }
    }
    ...
    void parse_field(vtag_t<field::genotypes> const & /**/,
                     var_io::genotypes_vcf_style<ownership::shallow> & parsed_field)
    {
        parse_genotypes_impl_vcf_style(shallow_genotypes_cache, parsed_field);
    }

    //!\overload
    void parse_field(vtag_t<field::genotypes> const & /**/, var_io::genotypes_vcf_style<ownership::deep> & parsed_field)
    {
