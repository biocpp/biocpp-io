...
// #include <bio/detail/ignore_output_iterator.hpp>
...
private:
    ...
    void parse_field(vtag_t<field::chrom> const & /**/, parsed_field_t & parsed_field)
    {
    ...
    void parse_field(vtag_t<field::info> const & /**/, std::vector<std::pair<key_t, value_t>> & parsed_field)
    {
        static_assert(detail::one_of<key_t, int32_t, std::string, std::string_view>,
                      "The key type for reading INFO fields must be int32_t, std::string or std::string_view.");
        static_assert(detail::is_dynamic_type<value_t> || detail::one_of<value_t, std::string, std::string_view>,
    ...
    template <typename field_t>
        requires(std::same_as<std::remove_cvref_t<field_t>, var_io::genotypes_bcf_style<ownership::shallow>> ||
                 std::same_as<std::remove_cvref_t<field_t>, var_io::genotypes_bcf_style<ownership::deep>>)
    void parse_field(vtag_t<field::genotypes> const & /**/, field_t & parsed_field)
    {
        ...
        for (std::string_view format_name : format_names | detail::eager_split(':'))
        {
            ...
            parsed_field.push_back({header.formats[format_pos].idx, {}});

            auto const & format = header.formats[format_pos];

            detail::init_dynamic_type(format.type, parsed_field.back().second);
        ...
        for (size_t i = 9; i < column_number; ++i)
        {
            ...
            for (size_t j = 0; j < formats; ++j)
            {
                ...
                if (fields_it != std::default_sentinel)
                {
                    ...
                    std::visit(parse_and_append, parsed_field[j].second);
                }
    ...
    void parse_field(vtag_t<field::genotypes> const & /**/, field_t & parsed_field)
    {
        ...
        using string_t  = std::remove_cvref_t<decltype(parsed_field.first[0])>;
        using variant_t = std::remove_cvref_t<decltype(parsed_field.second[0][0])>;
