...
//!\brief Can be included as a member to indicate whether parent is in moved-from-state.
//!\ingroup bio
struct move_tracker
{
    move_tracker() = default;           //!< Defaulted.
    move_tracker(move_tracker && rhs)   //!< Sets moved-from-state.
    {
        rhs.moved_from = true;
    }
    move_tracker & operator=(move_tracker && rhs) //!< Sets moved-from-state.
    {
        rhs.moved_from = true;
        return *this;
    }
